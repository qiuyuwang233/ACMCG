...
def clustering_loop(feature_space, dict_mapping, skeleton):
    ...
    for sub_S in S:
        ...
        need_compute = False
        for p in points:
            if p in affected_nodes or p not in skeleton.wslc_cc:
                need_compute = True
                break
        ...
        if need_compute:
            ...
            for idx, node in enumerate(local_nodes):
                skeleton.wslc_cc[node] = wslc_scores[idx]

        score1 = skeleton.wslc_cc.get(points[0], 0)
        score2 = skeleton.wslc_cc.get(points[1], 0)
